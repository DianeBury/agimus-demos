...
methods = ['complete', 'basic', 'memory', 'vmax', 'sorting', 'discrete']
...
def run(method='Complete', time_step=0):
    if method.lower() not in methods:
        print("Input method is not in the acceptable methods")
        return False
    try:
        import rospy
    ...
    if method.lower() in ["discrete"]:
        print("Using path validation method: Discretized, time step = " + str(time_step))
        ps.selectPathValidation("Graph-Discretized", time_step)
    else:
        print("Using path validation method: "+method.lower().title())
        if method.lower() in ["complete"]:
    ...
    class InStatePlanner:
        ...
        def solveTSP(self, configs, resetRoadmapEachTime, pb_kwargs={}):
            ...
            for i,j in zip(permutation,permutation[1:]):
                p = paths[i][j] if i < j else wd(paths[j][i].reverse()) if paths[j][i] else None
                solution.append(p)
    ...
    def concatenate_paths(paths):
        ...
        while not done:
            if paths[index] is not None:
                p = paths[index].asVector()
                done = True
                print("First non None index: " + str(index))
            index += 1
        for q in paths[index+1:]:
            if q is not None:
                p.appendPath(q)
        return p
    # 2}}}
    ...
    #         paths_order.append(clusers_path[i])
    # add_paths(paths_order)

    return ps, base_order, base_paths, clusters_path
