...
vf.loadRobotModel (PartP72, "table")
...
def testPath(q1,q2):
    ...
    times = {}
    # # Test BASIC
    # ps.selectPathValidation("Graph-DichotomyBasic", 0)
    # graph.initialize()
    # res, time = validatePath(q1,q2)
    # times['BASIC'] = time
    # valid['BASIC'] = res
    # # Test MEMORY
    # ps.selectPathValidation("Graph-DichotomyMemory", 0)
    # graph.initialize()
    # res, time = validatePath(q1,q2)
    # times['MEMORY'] = time
    # valid['MEMORY'] = res
    # # Test VMAX
    # ps.selectPathValidation("Graph-DichotomyVmax", 0)
    # graph.initialize()
    # res, time = validatePath(q1,q2)
    # times['VMAX'] = time
    # valid['VMAX'] = res
    # # Test SORTING
    # ps.selectPathValidation("Graph-DichotomySorting", 0)
    # graph.initialize()
    # res, time = validatePath(q1,q2)
    # times['SORTING'] = time
    # valid['SORTING'] = res
...
def testPathDiscrete(q1, q2, time_steps=[]):
    valid = {}
    times = {}
    # Test DISCRETE
    for step in time_steps:
        ps.selectPathValidation("Discretized", step)
        graph.initialize()
        res, time = validatePath(q1,q2)
        times['Discrete'+str(step)] = time
        valid['Discrete'+str(step)] = res
    return valid, times
...
def add_new_times(all_times, new_times):
    for key in all_times:
        if key in new_times:
            all_times[key].append(new_times[key])
    for key in new_times:
        if not key in all_times:
            all_times[key] = [new_times[key]]

def increment_dict(main_dict, new_dict):
    for key in main_dict:
        if key in new_dict:
            main_dict[key] += 1 if new_dict[key] else 0
    for key in new_dict:
        if not key in main_dict:
            main_dict[key] = 1 if new_dict[key] else 0

def launch_tests(Ntests = 1, case='FullBody', time_steps=[]):
    ...
    for i in range(Ntests):
        ...
        if res:
            ...
            if all_valid(results):
                valid_paths['CONTINUOUS'] += 1
            results_d, times_d = testPathDiscrete(q1, q2, time_steps)
            # print("results discrete : ", results_d)
            increment_dict(valid_paths, results_d)
            add_new_times(all_times, times)
            add_new_times(all_times, times_d)
...
def launch_all_tests(Ntests=1, write=True, time_steps=[]):
    results = {}
    # Arm Neutral test
    ...
    # print()
    # FullBody test
    configs, valid_paths, times = launch_tests(Ntests, case='FullBody', time_steps=time_steps)
    print(valid_paths, "valid paths out of", Ntests)
    times = {key:value for (key, value) in times.items() if len(value) >0}
    results['FullBody'] = times #get_stats(times)
    if write:
...
def launch_benchmark_and_analyse(Ntests=1000, write=False, time_steps=[]):
    print("Benchmark for continuous methods and discrete with time_steps = ", time_steps)
    times = launch_all_tests(Ntests, write=write, time_steps=time_steps)
    # Finding the file log and moving it
    list_of_files = glob.glob(devel_dir + 'install/var/log/hpp/benchmark.*.log')
    if len(list_of_files) == 0:
        print("No log file in " + devel_dir + 'install/var/log/hpp/benchmark.*.log')
        print("Is logging activated ?")
        return None, None
    else:
        latest_file = max(list_of_files, key=os.path.getctime)
        pid = latest_file.split('benchmark.')[1].split('.log')[0]
        new_filename = devel_dir + '/src/agimus-demos/tiago/deburring/data/simple_benchmark_' + pid + '.log'
        print("Moving log file (" + latest_file +") to data folder (" + new_filename + ")")
        os.rename(latest_file, new_filename)
        counts, stats = analyse_log.analyse(new_filename, time_steps=time_steps)
    return times, stats
...
if __name__=='__main__':
    ...
    ps.setRandomSeed(random_seed)
    times, iterations = launch_benchmark_and_analyse(10, write=False, time_steps=[0.12])
# 2}}}
